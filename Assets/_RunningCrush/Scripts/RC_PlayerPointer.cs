...
Photo Order Layer will minus one in case the photo does not be ontop of the frame.")]
    ...
    private void Start()
    {
        ...
        if (RC_GameSettings.FirstInstance().WEBCAM_MODE &&
            RC_GameSettings.FirstInstance().GAME_MODE != RC_GameMode.SINGLE_PLAYERS)
        {
            ...
            {
                Debug.LogError("No Photo sprite renderer assigned");
            }
        }
